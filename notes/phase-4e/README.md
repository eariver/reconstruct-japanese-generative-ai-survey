...
**2026-09-13 JST更新:** 途中Push後にPR #488 mergeと新W34を照合。[production-refresh.json](production-refresh.json) / [refresh-check.json](refresh-check.json) / 判断書§6が現在の補足。bib note修復はmain/W34へ反映済みで重複実装しない。下記著述・function-unit出力は元refの歴史試験として保持する。

## 入力と著述範囲

- [初回probeが使用した固定入力](used-inputs.json)。rawは`.phase-4-inputs/<ref>/<path>`。初回の追加GETは4件、途中Push後の更新GETは11件で[inputs.json](inputs.json)に併記。他は4-Dから再利用。各rawのGit blob SHA-1 / SHA-256を検査。
...
python -X utf8 notes/phase-4e/check_refresh.py
```
...
途中Push後の`check_refresh.py`は固定main/W34の差分を対象に、serializer関数の差分、41 bibliography field値/順序、Candidate→4ファイルとmanuscript→bib/section20の名指しhashを検査する。PDF本体・全State/Gate閉包は実行しない。最新statusはCandidate recordとして読み、Human Previewの承認に置き換えない。
